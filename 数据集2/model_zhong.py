...
class TimeSeriesDataset(Dataset):
    def __init__(self, csv_file, feature_columns, label_column, seq_len=8, scaler=None):
        ...
        # 处理标签
        #label_map = {"BENIGN": 1, "DDoS": 0}
        #self.labels = self.data[label_column].map(label_map).values

        self.labels = self.data[label_column].apply(lambda x: 1 if x == "BENIGN" else 0).values
...
def main():
    # 定义特征和标签列
    feature_columns = [" Total Fwd Packets", " Flow Duration", " Bwd Packet Length Std"," Flow Packets/s", " Flow IAT Max", " Flow IAT Mean"]
    label_column = " Label"
    ...
    if not os.path.exists("transformer_model.pkl"):
        ...
        # 数据集路径
        dataset_csv = "Thursday-WorkingHours-Morning-WebAttacks.pcap_ISCX.csv"
        full_dataset = TimeSeriesDataset(dataset_csv, feature_columns, label_column, seq_len=8)
        ...
        # 加载数据集并随机划分
        dataset_csv = "Friday-WorkingHours-Afternoon-DDos.pcap_ISCX.csv"
